...
2. Genetic
- The genetic algorithm will ask the user how many "rounds" to run genetic. Each round represents one call to a crossover function and a certain percent chance (currently 10%) of one call to a mutation function. The user is asked which functions to run, as well as if they would like to enable/disable hill climbing after each population is generated, which increases time by a lot. If the user's inputs do not correspond to one of the given options, the option marked default is run. The genetic algorithm will start to require more time around 500 or 1000 rounds.
