...
def choose_option(g, start_end, locs, cities, e_file, solution_set, distance_set, time_set):
    """ User Interface """
    ...
    if option == 1:
        ...
        # update solution set
        solution_set = update(solution_set, 0, greedy_result[0])
        distance_set = update(distance_set, 0, greedy_result[1])
        time_set = update(time_set, 0, t1)
    ...
    # run genetic
        ...
        # update solution set
        solution_set = update(solution_set, 1, gen_result[0])
        distance_set = update(distance_set, 1, gen_result[1])
        gen_time = gen_result[2]
        time_set = update(time_set, 1, gen_time)
                             
        print "The best solution generated by the genetic algorithm has \n \
            distance of ", gen_result[1]
        print "Genetic algorithm took " + str(gen_time) + " seconds."
    ...
    # run dynamic
        ...
        # update solution set
        solution_set = update(solution_set, 2, dynamic_result[0])
        distance_set = update(distance_set, 2, dynamic_result[1])
        time_set = update(time_set, 2, t3)
    ...
    # print out current solutions
        ...
        if dist == []:
            t = time.time()
            res = tsp_dynamic(g, start_end, cities - 1, locs)
            t = round(time.time() - t, 3)
            dist = res[1]
            distance_set = update(distance_set, 2, res[1])
        ...
        f.write(alg_name + " run on " + str(cities) + ", distance of " + str(distance_set[y-1]) + ", percent error "
                + str(percent_error(distance_set[y-1], distance_set[2])) + "% with time " + str(time_set[y-1]) + " seconds.\n")
        f.close()
